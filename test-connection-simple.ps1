...
$testCmd = "sqlcmd -S `"$server`" -U `"$user`" -P `"$password`" -Q `"SELECT @@SERVERNAME AS ServerName, @@VERSION AS SQLServerVersion`" -l 30"
...
$winAuthCmd = "sqlcmd -S `"$server`" -E -Q `"SELECT @@SERVERNAME AS ServerName`" -l 30"
